...
export function usePersistentColumnWidths(tableRef, getState, getStorageKey) { // NOVALYFT
    // NOVALYFT: `const renderer = useComponent()` removed — it was only read by the native
    // magic-widths guard below, which we replace with an always-on block (see "Side effects").
    ...
    function forceColumnWidths() {
        ...
        if (!columnWidths) {
            // NOVALYFT: seed the first computation from persisted widths (when present), passing
            // them as `startingWidths` so Odoo's own min/max + 100%-fit math refits them to the
            // current container. Hash-gated so a changed column set ignores stale widths.
    ...
    // NOVALYFT: clear the persisted widths for this view, then recompute ideal widths.
    ...
    // Side effects
    // NOVALYFT: always register the column-width effects. Upstream gates these on
    // `renderer.constructor.useMagicColumnWidths`; our patch neuters that native flag so the
    // stock hook stays inert, and invokes THIS hook only when widths are wanted.
    { // NOVALYFT (upstream: `if (renderer.constructor.useMagicColumnWidths) {`)
